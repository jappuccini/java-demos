...
public class Example {
  ...
  public static void main(String[] args) {
    ...
    Stream<String> namesStream1 =
        names.stream().filter(name -> name.length() > 4).map(name -> name.toUpperCase()).limit(2);
    ...
    names.stream().limit(2).filter(name -> name.startsWith("M")).forEach(System.out::println);
    names.stream().filter(name -> name.startsWith("M")).limit(2).forEach(System.out::println);
  }
}
