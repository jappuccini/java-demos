...
public class CarStore {
  ...
  private Function<Car, String> carToKey =
      car -> car.brand().toLowerCase() + car.model().toLowerCase();
  private Function<Car, Car> carToValue = car -> car;

  public CarStore() {
    this.carMap = this.getCarStream().collect(Collectors.toMap(carToKey, carToValue));
  }
  ...
  private Stream<Car> getCarStream() {
    return Stream.of(
        new Car("Fiat", "500", 92), new Car("BMW", "320i", 184), new Car("Audi", "A4", 169));
  }
}
